...
#            confidence_complex_dict = {d.name: d for d in confidence_test_dataset}
    ...
    run_times, min_self_distances_list, without_train_overlap_list = [], [], []
    gnina_rmsds_list, gnina_score_list = [], []
    N = args.samples_per_complex

    # names_no_train_overlap = []
    # next time this is run, update this file to be the list of compounds that are not part of the training time-split: Eric 12/2/24
    names_no_train_overlap = read_strings_from_txt(f'/vast/eac709/allo_crossdock/splits/all_241120_timesplit_train.txt')
    ...
    for overlap in ['', 'no_overlap_']:
        if 'no_overlap_' == overlap:
            without_train_overlap = np.array(without_train_overlap_list, dtype=bool)
            if without_train_overlap.sum() == 0: continue
            rmsds = np.array(rmsds_list)[without_train_overlap]
            min_self_distances = np.array(min_self_distances_list)[without_train_overlap]
            centroid_distances = np.array(centroid_distances_list)[without_train_overlap]
            if args.confidence_model_dir is not None:
                confidences = np.array(confidences_list)[without_train_overlap]
            else:
                confidences = np.array(confidences_list)
            names = np.array(names_list)[without_train_overlap]
            gnina_rmsds = np.array(gnina_rmsds_list)[without_train_overlap] if args.gnina_minimize else None
            gnina_score = np.array(gnina_score_list)[without_train_overlap] if args.gnina_minimize else None
