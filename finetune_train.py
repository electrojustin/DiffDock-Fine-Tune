...
from utils.training import train_epoch, test_epoch, loss_function, loss_function_ddp
...
def get_dataset(args, model_args, transform):
    dataset = LazyPDBBindSet(transform=transform, root=args.pdbbind_dir, limit_complexes=args.limit_complexes,
...
def construct_datasets(args, t_to_sigma, model_args):
    ...
    target_dataset = get_dataset(args, model_args, None)
...
def inference_epoch(model, filtering_model, complex_graphs, filtering_complex_dict, device, t_to_sigma, args, filtering_args, confidence_cutoff, score_model_args):
    ...
    for orig_complex_graph in tqdm(loader):
        ...
        if filtering_model is not None and not (
            ...
            filtering_complex = next(iter(GeometricDataLoader([filtering_complex_dict.get_by_name(orig_complex_graph.name[0])], batch_size=1)))
        ...
        confidence_data_list = [copy.deepcopy(filtering_complex) for _ in range(args.inference_samples)]
        ...
        while predictions_list == None:
            try:
                predictions_list, confidences = sampling(data_list=data_list, model=model,
                                                     ...
                                                     confidence_model_args=filtering_args,
                                                     t_schedule=t_schedule,
                                                     batch_size=bs, pivot=None, no_final_step_noise=True,
        ...
        for i in range(len(orig_ligand_pos)):
            try:
                ...
                print("Using non corrected RMSD because of the error: " + str(e))
...
def inference_finetune(args, model, filtering_model, filtering_args, filtering_complex_dict, confidence_cutoff, 
                       optimizer, ema_weights, finetune_dataset, target_loader, t_to_sigma, run_dir, score_model_args):
    ...
    for epoch in range(args.n_epochs):
        ...
        if epoch % args.cb_inference_freq == 0:
            ...
            for i in range(iterations):
                inf_m, compl, filtered_rmsds = inference_epoch(model, filtering_model, inf_dataset, filtering_complex_dict, device, t_to_sigma, args,
                                                            filtering_args, confidence_cutoff, score_model_args)
            ...
            finetune_loader = DataLoader(dataset=finetune_dataset, batch_size=args.batch_size, num_workers=args.num_dataloader_workers, shuffle=True, pin_memory=args.pin_memory, drop_last=args.dataloader_drop_last, collate_fn=lambda batch: [x for x in batch if x is not None])
        ...
        if args.use_ema: ema_weights.copy_to(model.parameters())
        ema_state_dict = copy.deepcopy(model.state_dict() if device.type == 'cuda' else model.state_dict())
        ...
        train_losses = train_epoch(model, finetune_loader, optimizer, device, t_to_sigma, loss_fn, ema_weights)
...
def main_function():
    ...
    if args.wandb:
        wandb.init(
            entity='jeg10045-new-york-university',
    ...
    with open(f'{args.pretrain_dir}/model_parameters.yml') as f:
        score_args = Namespace(**yaml.full_load(f))
        score_args.DDP = False
    # Construct datasets
    ...
    state_dict = torch.load(f'{args.pretrain_dir}/{args.pretrain_ckpt}.pt', map_location=torch.device('cpu'))
    corrected_state_dict = {}
    for key in state_dict.keys():
        corrected_state_dict[key.replace('module.', '')] = state_dict[key]
    state_dict = corrected_state_dict
    if args.use_ema:
        ema_weights.load_state_dict(state_dict, device=device)
    ...
    model.eval()
    finetune_dataset, target_loader = construct_datasets(args, t_to_sigma, score_args)
    ...
    if not hasattr(score_args, 'lm_embeddings_path'):
         score_args.lm_embeddings_path = None
    if not hasattr(score_args, 'correct_torsion_sigmas'):
         score_args.correct_torsion_sigmas = False
    if not hasattr(score_args, 'esm_embeddings_path'):
         score_args.esm_embeddings_path = None
    ...
    if not (filtering_args.use_original_model_cache or filtering_args.transfer_weights):
        ...
        filtering_test_dataset = get_dataset(args, filtering_args, None)
        filtering_complex_dict = filtering_test_dataset
    ...
    filtering_model_args.DDP = False
    ...
    save_yaml_file(yaml_file_name, score_args.__dict__)
    args.device = device

    inference_finetune(args, model, filtering_model, filtering_model_args, filtering_complex_dict, args.confidence_cutoff,
                       optimizer, ema_weights, finetune_dataset, target_loader, t_to_sigma, run_dir, score_args)
